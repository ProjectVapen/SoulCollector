...
MyDirect3D::MyDirect3D()
{
	...
	SecureZeroMemory(this, sizeof(MyDirect3D));
}
...
HRESULT MyDirect3D::Init(sD3D_Init* pcd){
	Window _window;
	m_hWnd = pcd->hWnd;
	...
	sd.BufferCount = 1;
	sd.BufferDesc.Width = _window.WindowWidth();
	sd.BufferDesc.Height = _window.WindowHeight();
	...
	descDepth.Width = _window.WindowWidth();
	descDepth.Height = _window.WindowHeight();
	...
	//ビューポートの設定
	D3D11_VIEWPORT vp;
	vp.Width = _window.WindowWidth();
	vp.Height = _window.WindowHeight();
	vp.MinDepth = 0.0f;
	vp.MaxDepth = 1.0f;
	vp.TopLeftX = 0;
	vp.TopLeftY = 0;
